...
def run_noise(datasets=None, config=None, callbacks=None):
    if datasets is None:
    ...
    csv_path = 'results/metrics/noise_analysis.csv'
    all_results = []
    existing_df = None
    ...
    for ds_name in datasets:
        print(f"\n{'=' * 50}")
        print(f'Running NOISE Analysis on: {ds_name.upper()}')
        print(f"{'=' * 50}")
        ...
        for noise in noise_levels:
            ...
            models = {
                ...
                'Ananta Legacy': AnantaVQC(n_qubits=n_features, epochs=config.get('epochs', 20), noise_prob=noise, callbacks=callbacks),
                'Ananta Pro': AnantaPro(n_qubits=n_features, head_search='fast', quantum_feature_seeds=(11,), noise_aware=True, noise_prob=noise, callbacks=callbacks),
                'Samyoga Legacy': SamyogaLegacySVM(n_qubits=n_features, epochs_pretrain=config.get('epochs', 10), dataset_name=ds_name, noise_prob=noise, tuning_mode='fast', callbacks=callbacks),
            ...
            for model_name, model in models.items():
                if existing_df is not None:
                    match = existing_df[(existing_df['Dataset'] == ds_name) & (existing_df['Model'] == model_name) & ((existing_df['Noise'] == noise) if 'Noise' in existing_df.columns else (existing_df['Noise_Prob'] == noise))]
                ...
                try:
                    ...
                    all_results.append({'Dataset': ds_name, 'Model': model_name, 'Noise': noise, 'Accuracy': res['accuracy'], 'AUC_ROC': res['auc_roc']})
                    print(f"  Accuracy: {res['accuracy']:.4f}")
                except Exception as e:
                    print(f'  [ERROR] {model_name} failed: {e}')
    run_df = pd.DataFrame(all_results)
    if existing_df is not None and not existing_df.empty:
        noise_col = 'Noise' if 'Noise' in existing_df.columns else 'Noise_Prob'
        df = pd.concat([existing_df, run_df], ignore_index=True)
        df = df.drop_duplicates(subset=['Dataset', 'Model', noise_col], keep='last')
    else:
        df = run_df
    if not df.empty:
        df.to_csv(csv_path, index=False)
        print(f'\nNoise results saved to {csv_path}')
    return run_df

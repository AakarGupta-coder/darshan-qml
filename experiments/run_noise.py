    ...
    for ds_name in datasets:
        ...
        for noise in noise_levels:
            ...
            models = {
                ...
                "Ananta Legacy": AnantaVQC(
                    n_qubits=n_features, epochs=config.get("epochs", 20), noise_prob=noise, callbacks=callbacks
                ),
                "Ananta Pro": AnantaPro(
                    n_qubits=n_features,
                    head_search="fast",
                    quantum_feature_seeds=(11,),
                    noise_aware=True,
                    noise_prob=noise,
                    callbacks=callbacks,
                ),
            ...
            for model_name, model in models.items():
                if existing_df is not None:
                    match = existing_df[
                        ...
                        & (
                            (existing_df["Noise"] == noise)
                            if "Noise" in existing_df.columns
                            else (existing_df["Noise_Prob"] == noise)
                        )
                    ]

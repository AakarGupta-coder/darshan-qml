...
def run_noise_study(dataset_name='iris', noise_levels=[0.0, 0.01, 0.05, 0.1], epochs_vqc=50):
    ...
    for p in noise_levels:
        ...
        models = {
            ...
            'Ananta Pro': AnantaPro(n_qubits=n_features, head_search='fast', quantum_feature_seeds=(11,), noise_aware=True, noise_prob=p),
            'Samyoga Legacy': SamyogaLegacySVM(n_qubits=n_features, epochs_pretrain=10, dataset_name=dataset_name, noise_prob=p, tuning_mode='fast'),
    ...
    if not classical_rows.empty:
        ...
        plt.axhline(y=classical_accuracy, color='black', linestyle='--', label=f'Classical SVM (Noiseless Baseline)')

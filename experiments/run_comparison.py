...
def run_comparison(datasets=None, mode='full', fair_mode=False, config=None, callbacks=None):
    ...
    if existing_df is not None:
        ...
        if trained_models:
            ...
            for m in trained_models:
                if 'Samyoga' in m: families['Samyoga'].append(m)
                elif 'Parampara' in m: families['Parampara'].append(m)
                elif 'Ananta' in m: families['Ananta'].append(m)
                else: families['Samyoga'].append(m)
    ...
    for ds_name in datasets:
        ...
        for seed in seeds:
            ...
            models = {}
            models['Parampara Legacy'] = ParamparaLegacy(tuning_mode='fast') if (mode in ['fast', 'low_data', 'quantum_safe'] and len(full_data['X_train']) > max_samples) else ParamparaLegacy(tuning_mode=mode if mode in ['fast', 'full', 'research'] else 'fast')
            ...
            models['Ananta Legacy'] = AnantaVQC(n_qubits=n_features, epochs=config.get('epochs', 30), callbacks=callbacks)
            models['Ananta Pro'] = AnantaPro(n_qubits=n_features, quantum_feature_seeds=(11, 23, 37) if p_mode == 'research' else (42,), head_search=p_mode, callbacks=callbacks)

            models['Samyoga Legacy'] = SamyogaLegacySVM(n_qubits=n_features, epochs_pretrain=config.get('epochs', 15), dataset_name=ds_name, callbacks=callbacks, random_state=seed, tuning_mode='fast' if mode == 'fast' else 'full')
            if fair_mode:
                models['Samyoga Shadow'] = SamyogaShadow(d_model=n_features, d_state=8, num_experts=3, callbacks=callbacks)
            models['Samyoga Pro'] = SamyogaPro(d_model=n_features, d_state=8, qubits=4, num_experts=3, callbacks=callbacks)
            models['Samyoga Go'] = SamyogaGo(d_model=n_features, d_state=8, qubits=4, num_experts=3, callbacks=callbacks)

            for model_name, model in models.items():
                skip = False
                if existing_df is not None:
                    match = existing_df[(existing_df['Dataset'] == ds_name) & (existing_df['Seed'] == seed) & (existing_df['Model'] == model_name)]
                ...
                try:
                    ...
                    record = {'Dataset': ds_name, 'Seed': seed, 'Model': model_name, 'Accuracy': res['accuracy'], 'F1_Macro': res['f1_macro'], 'AUC_ROC': res['auc_roc'], 'Brier_Score': res.get('brier_score', float('nan')), 'Log_Loss': res.get('log_loss', float('nan')), 'Train_Time_s': res['train_time'], 'Predict_Time_s': res.get('predict_time', float('nan'))}

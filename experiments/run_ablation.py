...
def run_ablation(datasets=None, config=None, callbacks=None):
    ...
    for ds_name in datasets:
        ...
        models = {'Samyoga Legacy (Full)': SamyogaLegacySVM(n_qubits=n_features, epochs_pretrain=config.get('epochs', 10), use_interactions=True, feature_selection=True, tuning_mode='fast', callbacks=callbacks), 'Samyoga Legacy (No Interactions)': SamyogaLegacySVM(n_qubits=n_features, epochs_pretrain=config.get('epochs', 10), use_interactions=False, feature_selection=True, tuning_mode='fast', callbacks=callbacks), 'Samyoga Legacy (No Selection)': SamyogaLegacySVM(n_qubits=n_features, epochs_pretrain=config.get('epochs', 10), use_interactions=True, feature_selection=False, tuning_mode='fast', callbacks=callbacks), 'AnantaPro (Full Hybrid)': AnantaPro(n_qubits=n_features, ablation_mode=None, use_interactions=True, head_search='fast', callbacks=callbacks), 'AnantaPro (Classical Only)': AnantaPro(n_qubits=n_features, ablation_mode='classical_only', head_search='fast', callbacks=callbacks), 'AnantaPro (Quantum Only)': AnantaPro(n_qubits=n_features, ablation_mode='quantum_only', head_search='fast', callbacks=callbacks), 'AnantaPro (Shuffled Quantum)': AnantaPro(n_qubits=n_features, ablation_mode='shuffled_quantum', head_search='fast', callbacks=callbacks), 'AnantaPro (No Interactions)': AnantaPro(n_qubits=n_features, ablation_mode=None, use_interactions=False, head_search='fast', callbacks=callbacks)}
        for model_name, model in models.items():
            ...
            try:
                ...
                all_results.append({'Dataset': ds_name, 'Model': model_name, 'Accuracy': res['accuracy'], 'AUC_ROC': res['auc_roc']})

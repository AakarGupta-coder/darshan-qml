...
def generate_statistics_report(csv_path):
    ...
    print(f"{'=' * 50}")
    summary = df.groupby(['Dataset', 'Model']).agg(Accuracy_Mean=('Accuracy', 'mean'), Accuracy_Std=('Accuracy', 'std'), AUC_ROC_Mean=('AUC_ROC', 'mean'), AUC_ROC_Std=('AUC_ROC', 'std')).reset_index()
    print('\nAggregate Metrics:')

from utils.config import darshan_config

STATE = {
    "theme": darshan_config.get("app", {}).get("theme", "cyberpunk"),
    "dataset": None,
    "epochs": 10,
    "working_mode": False,
    "quiet_mode": False,
    ...
    "last_figures_path": None,
}
THEMES = {
    "neon": {
        "primary": "bold #FF107A",
        "secondary": "bold #00E5FF",
        "accent": "bold #7000FF",
        "success": "bold #00FF9D",
        "warning": "bold #FFB300",
        "error": "bold #FF0055",
...
def get_model_color(model_name: str) -> str:
    ...
    if "ananta" in name:
        ...
        palette = [
            get_color(k).replace("bold ", "").replace("dim ", "")
            for k in ["primary", "secondary", "accent", "success", "warning", "error"]
        ]
...
def get_q_style():
    ...
    return prompt_toolkit.styles.Style(
        [
            ("qmark", f"fg:{p} bold"),
            ("question", f"bold fg:{p}"),
            ("answer", f"fg:{a} bold"),
            ("pointer", f"fg:{s} bold"),
            ("highlighted", f"fg:{p} bold"),
            ("selected", f"fg:{s}"),
            ("separator", f"fg:{m}"),
            ("instruction", f"fg:{m} italic"),
            ("text", ""),
        ]
    )
...
def gradient_text(text, start_hex, end_hex):
    ...
    lines = text.split("\n")
    max_len = max((len(lbl) for lbl in lines), default=1)
    if max_len <= 1:
        max_len = 2
    out_lines = []
    for line in lines:
...
def animate_panel(text, title, border_style, delay=0.005, header=None, footer=None):
    ...
    with Live(
        Panel(Group(*initial_items), title=title, border_style=border_style),
        refresh_per_second=60,
        console=_anim_console,
    ) as live:
        for char in text:
            current_text += char
            items = []

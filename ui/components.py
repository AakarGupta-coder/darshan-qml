...
def df_to_table(df, title, header_style_key="header"):
    ...
    abbrevs = {"Train_Time_s": "TTS", "Predict_Time_s": "PTS", "Efficiency": "Eff", "F1_Macro": "F1"}
    used_abbrevs = {}
    cols = []
...
def animate_gradient_logo():
    ...
    from ui.theme import LOGO as MAIN_LOGO
    ...
    progress = Progress(
        SpinnerColumn(spinner_name="aesthetic", style=f"bold {c_pri}"),
        TextColumn("[progress.description]{task.description}"),
        BarColumn(bar_width=40, complete_style=c_acc, finished_style=c_pri, pulse_style="#ffffff"),
        TextColumn("[progress.percentage]{task.percentage:>3.0f}%"),
        transient=True,
    )
    task_id = progress.add_task("[dim cyan]Booting Darshan Quantum Kernel...", total=len(colored_lines))
    current = Text()
    with Live(Align.center(Group(current, progress)), refresh_per_second=30, transient=True):

...
def render_rich_metric_bars(
    ...
    table = Table(
        show_header=True, header_style=get_color("header"), box=box.HEAVY_EDGE, border_style=get_color("primary")
    )
    ...
    if compact_legend:
        ...
        samyoga_present = any("samyoga" in lbl.lower() for lbl in labels)
        parampara_present = any("parampara" in lbl.lower() for lbl in labels)
        ananta_present = any("ananta" in lbl.lower() for lbl in labels)
        legend_text = ""
        if samyoga_present:
            legend_text += "[bold]Samyoga Family[/bold]\n"
            if any("go" in lbl.lower() or "fast" in lbl.lower() for lbl in labels):
                legend_text += f" Go/Fast:   {gt('samyoga go')}\n"
            if any("shadow" in lbl.lower() for lbl in labels):
                legend_text += f" Shadow:    {gt('samyoga shadow')}\n"
            if any("pro" in lbl.lower() and "pro+" not in lbl.lower() for lbl in labels):
                legend_text += f" Pro:       {gt('samyoga pro')}\n"
            if any("legacy" in lbl.lower() for lbl in labels):
                legend_text += f" Legacy:    {gt('samyoga legacy')}\n"
            legend_text += "\n"
        if parampara_present:
            legend_text += "[bold]Parampara Family[/bold]\n"
            if any("pro+" in lbl.lower() for lbl in labels):
                legend_text += f" Pro+:      {gt('parampara pro+')}\n"
            if any("pro" in lbl.lower() and "pro+" not in lbl.lower() for lbl in labels):
                legend_text += f" Pro:       {gt('parampara pro')}\n"
            if any("legacy" in lbl.lower() for lbl in labels):
                legend_text += f" Legacy:    {gt('parampara legacy')}\n"
            legend_text += "\n"
        if ananta_present:
            legend_text += "[bold]Ananta Family[/bold]\n"
            if any("pro" in lbl.lower() for lbl in labels):
                legend_text += f" Pro:       {gt('ananta pro')}\n"
            if any("legacy" in lbl.lower() for lbl in labels):
                legend_text += f" Legacy:    {gt('ananta legacy')}\n"
    ...
    main_panel = Panel(
        Columns([table, padded_legend], align="center", expand=False),
...
def generate_horizontal_legend(labels):
    ...
    legend_parts = []
    for lbl in list(dict.fromkeys(labels)):
        legend_parts.append(f"[bold]{lbl}[/bold]: {gt(lbl)}")
...
def render_terminal_curve(title, x_values, y_dict, x_title="X", y_title="Y"):
    ...
    if y_range < 0.15:
        ...
        fmt_str = "{:6.2f}"
    cfg = {"height": chart_height, "colors": colors_list, "format": fmt_str}

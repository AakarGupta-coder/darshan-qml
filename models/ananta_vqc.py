...
class AnantaVQC(BaseEstimator, ClassifierMixin):
    ...
    def _initialize_weights(self):
        ...
        return (
            qml.numpy.array(weights, requires_grad=True),
            qml.numpy.array(readout_w, requires_grad=True),
            qml.numpy.array(readout_b, requires_grad=True),
        )
    ...
    def fit(self, X, y, additional_epochs=None):
        ...
        for epoch in range(epochs_to_run):
            ...
            self._trigger_callback(
                "on_epoch_end", epoch=epoch, loss=epoch_loss, model_name="AnantaVQC", total_epochs=epochs_to_run
            )
    ...
    def save_checkpoint(self, path):
        ...
        np.savez(
            path,
            weights=self.weights.numpy() if hasattr(self.weights, "numpy") else self.weights,
            readout_w=self.readout_w.numpy() if hasattr(self.readout_w, "numpy") else self.readout_w,
            readout_b=self.readout_b.numpy() if hasattr(self.readout_b, "numpy") else self.readout_b,

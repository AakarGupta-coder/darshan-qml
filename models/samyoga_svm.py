...
class SamyogaLegacySVM(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, n_qubits, n_layers=3, learning_rate=0.05, epochs_pretrain=15, batch_size=32, noise_prob=0.0, cv_folds=5, random_state=42, max_train_samples=None, dataset_name='unknown', callbacks=None, use_interactions=True, feature_selection=True, calibration=True, tuning_mode='fast', warm_start=False, n_quantum_seeds=1, use_contrastive_loss=True, use_pca_hybrid=False):
        ...
        def _qnode(inputs, weights):
            ...
            return [qml.expval(qml.PauliX(i)) for i in range(self.n_qubits)] + [qml.expval(qml.PauliY(i)) for i in range(self.n_qubits)] + [qml.expval(qml.PauliZ(i)) for i in range(self.n_qubits)]
        self.qnode = _qnode
        self.weights = None
        self.readout_w = None
        self.readout_b = None
    ...
    def _get_cache_path(self, X, y=None):
        config = {'dataset': self.dataset_name, 'q': self.n_qubits, 'l': self.n_layers, 'ep': self.epochs_pretrain, 'rs': self.random_state, 'noise': self.noise_prob, 'interact': self.use_interactions}
    ...
    def fit(self, X, y):
        ...
        for s in range(self.n_quantum_seeds):
            ...
            for epoch in range(self.epochs_pretrain):
                ...
                for i in range(0, len(X_pre), self.batch_size):
                    ...
                    params = opt.step(lambda w, rw, rb: cost_wrapper(w, rw, rb, batch_X, batch_y), self.weights[s], self.readout_w[s], self.readout_b[s])
                ...
                self._trigger_callback('on_epoch_end', epoch=epoch, loss=epoch_loss, model_name=f'Samyoga Legacy Q-Encoder (Seed {s + 1})', total_epochs=self.epochs_pretrain)
        ...
        heads = {'SVM': (SVC(probability=True, random_state=self.random_state), {'C': [0.1, 1, 10], 'gamma': ['scale', 0.1], 'kernel': ['rbf', 'linear']}), 'LR': (LogisticRegression(max_iter=1000, random_state=self.random_state), {'C': [0.1, 1, 10]}), 'RF': (RandomForestClassifier(random_state=self.random_state), {'n_estimators': [50, 100], 'max_depth': [None, 5, 10]}), 'GB': (GradientBoostingClassifier(random_state=self.random_state), {'n_estimators': [50], 'learning_rate': [0.1, 0.05]}), 'ET': (ExtraTreesClassifier(random_state=self.random_state), {'n_estimators': [50, 100]}), 'Nystroem+SGD': (Pipeline([('nystroem', Nystroem(gamma=0.2, random_state=self.random_state, n_components=100)), ('sgd', SGDClassifier(loss='log_loss', max_iter=1000, random_state=self.random_state))]), {'nystroem__gamma': [0.1, 0.2, 0.5], 'sgd__alpha': [0.0001, 0.001, 0.01]})}
        if self.tuning_mode == 'fast':
            heads = {'SVM': (SVC(probability=True, random_state=self.random_state, kernel='rbf', gamma='scale'), {'C': [1, 10]})}
        best_score = -1
        best_head_name = None
        best_estimator = None
        ...
        cv = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=self.random_state) if use_stratified else KFold(n_splits=max(2, min(self.cv_folds, len(y))), shuffle=True, random_state=self.random_state)
    ...
    def get_training_summary(self):
        return {'total_epochs': self.total_epochs_trained_, 'training_sessions': self.training_sessions_, 'best_head': self.selected_head_name_, 'best_epoch': self.best_epoch_, 'quantum_seeds': self.n_quantum_seeds, 'classes': len(self.classes_) if self.classes_ is not None else 0}
    ...
    def save_checkpoint(self, path):
        ...
        np.savez(path, weights=w_list, readout_w=rw_list, readout_b=rb_list, classes=self.classes_, total_epochs_trained=self.total_epochs_trained_, loss_history=self.loss_history_, val_score_history=self.val_score_history_, train_score_history=self.train_score_history_, training_sessions=self.training_sessions_)
    ...
    def evaluate(self, X_test, y_test):
        ...
        try:
            ...
            ll = float('nan')
        return {'model': f'Samyoga Legacy (Hybrid Ensemble - {self.selected_head_name_})', 'accuracy': accuracy_score(y_test, y_pred), 'f1_macro': f1_score(y_test, y_pred, average='macro'), 'auc_roc': auc, 'brier_score': brier, 'log_loss': ll, 'train_time': self.train_time_, 'predict_time': predict_time, 'selected_head': self.selected_head_name_, 'best_params': self.best_params_, 'classification_report': classification_report(y_test, y_pred, output_dict=True), 'loss_history': self.loss_history_}

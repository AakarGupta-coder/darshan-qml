...
class ParamparaPro(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        if self.mode == "industry" and n_samples > 10000:
            ...
            param_grid = {
                "classifier__learning_rate": loguniform(0.001, 1.0),
                "classifier__max_iter": [100, 200, 500, 1000],
                "classifier__max_leaf_nodes": [15, 31, 63, 127],
                "classifier__l2_regularization": loguniform(1e-06, 10.0),
                "classifier__min_samples_leaf": [1, 5, 10, 20, 50],
            }
        else:
            clf = SVC(kernel="rbf", probability=True, random_state=42)
            param_grid = {
                "classifier__C": loguniform(0.001, 1000.0),
                "classifier__gamma": loguniform(0.0001, 100.0),
        ...
        if self.tuning_mode == "fast":
            ...
            cv_strategy = (
                RepeatedStratifiedKFold(n_splits=n_splits, n_repeats=2, random_state=42)
                if use_stratified
                else KFold(n_splits=max(2, min(5, len(y))), shuffle=True, random_state=42)
            )
            n_iter = 100
            ...
            cv_strategy = (
                RepeatedStratifiedKFold(n_splits=n_splits, n_repeats=5, random_state=42)
                if use_stratified
                else KFold(n_splits=max(2, min(5, len(y))), shuffle=True, random_state=42)
            )
            n_iter = 500
    ...
    def evaluate(self, X_test, y_test):
        ...
        try:
            ...
            ll = float("nan")
        return {
            "model": f"ParamparaPro ({self.mode.capitalize()} Track)",
            ...
            "best_params": self.pipeline_.best_params_,
        }

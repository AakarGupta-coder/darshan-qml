...
class AnantaQuantumFeatures(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, n_qubits=4, quantum_feature_seeds=(11, 23, 37), observables=('X', 'Y', 'Z'), include_classical_features=True, noise_aware=False, noise_prob=0.005, ablation_mode=None, use_interactions=False, max_feasible_samples=50000, callbacks=None):
        ...
        self._feature_cache = {}
        self.diagnostics_ = {'q_feature_extraction_time': 0.0, 'cache_hits': 0, 'cache_misses': 0, 'n_circuit_executions': 0, 'circuit_depth_estimate': 0}
        self.dev = None
        self.qnode = None
    ...
    def fit(self, X, y=None):
        if len(X) > self.max_feasible_samples:
            raise MemoryError(f'Infeasible Kernel: Dataset size {len(X)} exceeds maximum feasible quantum samples ({self.max_feasible_samples}).')
        self._setup_quantum_backend()
        self.n_classical_features_ = X.shape[1]
        return self

    def transform(self, X):
        if self.ablation_mode == 'classical_only' or (not self.include_classical_features and self.ablation_mode != 'quantum_only'):
            pass
        should_be_classical_only = self.ablation_mode == 'classical_only'
        should_be_quantum_only = self.ablation_mode == 'quantum_only' or (not self.include_classical_features and self.ablation_mode not in ['shuffled_quantum', 'classical_only'])
...
class AnantaPro(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, n_qubits=4, quantum_feature_seeds=(11, 23, 37), observables=('X', 'Y', 'Z'), include_classical_features=True, head_search='research', noise_aware=False, noise_prob=0.005, ablation_mode=None, use_interactions=False, max_feasible_samples=50000, scoring='f1_macro', callbacks=None):
        self.n_qubits = n_qubits
        self.quantum_feature_seeds = quantum_feature_seeds
        self.observables = observables
        self.include_classical_features = include_classical_features
    ...
    def fit(self, X, y):
        ...
        q_features = AnantaQuantumFeatures(n_qubits=self.n_qubits, quantum_feature_seeds=self.quantum_feature_seeds, observables=self.observables, include_classical_features=self.include_classical_features, noise_aware=self.noise_aware, noise_prob=self.noise_prob, ablation_mode=self.ablation_mode, use_interactions=self.use_interactions, max_feasible_samples=self.max_feasible_samples, callbacks=self.callbacks)

        self.prep_pipeline_ = Pipeline([('scaler', StandardScaler()), ('pca', PCA(n_components=self.n_qubits)), ('q_features', q_features)])
        ...
        self.clf_pipeline_ = Pipeline([('select', SelectKBest(f_classif, k='all')), ('clf', SVC(probability=True, random_state=42))])
        param_grid = {'clf__C': loguniform(0.001, 1000.0), 'clf__gamma': loguniform(0.0001, 100.0), 'clf__class_weight': [None, 'balanced']}
        ...
        self.head_ = RandomizedSearchCV(self.clf_pipeline_, param_distributions=param_grid, n_iter=n_iter, cv=cv_strategy, n_jobs=None, random_state=42, scoring=self.scoring)
        ...
        self.diagnostics_ = {'n_qubits': self.n_qubits, 'quantum_seeds_used': len(self.quantum_feature_seeds), 'observables_extracted': self.observables, 'ablation_mode': self.ablation_mode, 'train_time': self.train_time_, 'best_head_params': self.head_.best_params_, 'head_search_scoring': self.scoring, 'noise_prob': q_feat_step.noise_prob, 'circuit_depth_estimate': q_feat_step.diagnostics_['circuit_depth_estimate'], 'n_circuit_executions': q_feat_step.diagnostics_['n_circuit_executions'], 'cache_hits': q_feat_step.diagnostics_['cache_hits'], 'cache_misses': q_feat_step.diagnostics_['cache_misses'], 'hardware_readiness_score': 'Moderate (Requires logical fault tolerance for high N, NISQ viable for N<200)'}
        ...
        if self.ablation_mode not in ['classical_only', 'quantum_only'] and self.include_classical_features:
            ...
            self.diagnostics_['claim_audit'] = {'classical_features_survived': classical_survived, 'quantum_features_survived': quantum_survived, 'quantum_advantage_plausible': quantum_survived > 0, 'details': f'{quantum_survived} quantum features and {classical_survived} classical features survived selection.'}
        return self
    ...
    def evaluate(self, X_test, y_test):
        ...
        try:
            ...
            ll = float('nan')
        return {'model': f'AnantaPro (Hybrid)', 'accuracy': accuracy_score(y_test, y_pred), 'f1_macro': f1_score(y_test, y_pred, average='macro'), 'auc_roc': auc, 'brier_score': brier, 'log_loss': ll, 'train_time': self.train_time_, 'predict_time': predict_time, 'diagnostics': self.diagnostics_}
...
class ParamparaLegacy(BaseEstimator, ClassifierMixin):
    ...
    def _get_param_grid(self):
        if self.tuning_mode == "fast":
            ...
            return {
                "svc__C": np.logspace(-2, 3, 6).tolist(),
                "svc__gamma": ["scale", "auto"] + np.logspace(-3, 1, 5).tolist(),
    ...
    def fit(self, X_train, y_train):
        ...
        if min_class_count >= 2:
            ...
            cv_strategy = StratifiedKFold(
                n_splits=min(self.cv_folds, min_class_count), shuffle=True, random_state=self.random_state
            )
        else:
            from sklearn.model_selection import KFold

            cv_strategy = KFold(
                n_splits=max(2, min(self.cv_folds, len(y_train))), shuffle=True, random_state=self.random_state
            )
        if self.tuning_mode in ["full", "large"]:
            ...
            search = GridSearchCV(
                estimator=pipeline, param_grid=param_grid, cv=cv_strategy, scoring="accuracy", n_jobs=None, refit=True
            )

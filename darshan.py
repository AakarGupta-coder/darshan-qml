...
from ui.theme import RAW_LOGOS, STATE, THEMES, apply_theme_to_prompt, get_color, get_q_style, model_color
...
def _make_progress():
    ...
    return Progress(
        SpinnerColumn(spinner_name="darshan_arrow", style=get_color("primary")),
        TextColumn(f"[{get_color('primary')}]{{task.description}} "),
        BarColumn(bar_width=30, complete_style=get_color("success"), finished_style=get_color("accent")),
        TextColumn(f"[{get_color('accent')}]" + "{task.percentage:>3.0f}%"),
        TimeElapsedColumn(),
        console=console,
        transient=False,
    )
...
class ModelProgressCallback:
    ...
    def on_stage_end(self, stage_name):
        self.progress.update(
            self.task_id, description=f"✓ {stage_name}", completed=self.progress.tasks[self.task_id].total
        )
    ...
    if not STATE["quiet_mode"]:
        with _make_progress() as progress:
            ...
            try:
                ...
                return None
            progress.update(task, description="[bold]✓ Complete![/bold]", completed=100, total=100)
        return result_df
    else:
        try:
...
def cmd_dataset(args):
    ...
    animate_panel(
        data["description"],
        title=f"{name.upper()} DATASET PROFILE",
        border_style=get_color("primary"),
        delay=0.01,
        footer=Group(" ", Align.center(table)),
    )
...
def cmd_train(args):
    ...
    if len(args) < 3 or args[0] != "continue":
        print_error_panel(
            "Invalid syntax", "Usage: /train continue <model> <epochs>", "Example: /train continue ananta 5", "/help"
        )
        return
    model_name = args[1].lower()
    if model_name not in ["ananta", "samyoga_legacy"]:
        print_error_panel(
            "Invalid model", "Only Ananta and Samyoga Legacy support progressive training.", "Check spelling.", "/help"
        )
        return
    try:
    ...
    print_panel(
        f"Resuming training for [bold]{model_name}[/bold] for {additional_epochs} epochs...", style_key="primary"
    )
    ...
    if df is not None:
        console.print(df_to_table(df, "CONTINUED TRAINING RESULT"))


def cmd_checkpoint(args):
    if len(args) < 2 or args[0] not in ["save", "load"]:
        print_error_panel(
            "Invalid syntax", "Usage: /checkpoint <save|load> <model>", "Example: /checkpoint save ananta", "/help"
        )
        return
    action = args[0]
    model_name = args[1].lower()
    if model_name not in ["ananta", "samyoga_legacy"]:
        print_error_panel(
            "Invalid model", "Only Ananta and Samyoga Legacy support checkpointing.", "Check spelling.", "/help"
        )
        return
    if action == "save":
        if model_name not in ACTIVE_MODELS:
            print_error_panel(
                "Model not active",
                f"{model_name} has not been initialized or trained yet.",
                "Train it first or load it.",
                "/test compare",
            )
            return
        ...
        if not os.path.exists(path):
            print_error_panel(
                "Checkpoint not found", f"No checkpoint exists at {path}", "Save one first.", "/checkpoint save"
            )
            return
        if not STATE["dataset"]:
            console.print(
                f"[{get_color('warning')}]No dataset loaded. Let's pick one first...[/{get_color('warning')}]"
            )
            cmd_dataset([])
            if not STATE["dataset"]:
                return
...
def cmd_model(args):
    if not args:
        choices = [
            "Samyoga (Hybrid Quantum Transfer Learning)",
            "Parampara (Classical RBF Support Vector Machine)",
            "Ananta (Quantum Classifiers)",
        ]
        choice_str = questionary.select(
            "Which model would you like to inspect?", choices=choices, style=get_q_style()
        ).ask()
        if not choice_str:
            return
        ...
        if name == "samyoga":
            sub_choice = questionary.select(
                "Which version of Samyoga?",
                choices=[
                    "Samyoga Pro (Enterprise-Grade QTL)",
                    "Samyoga Go (Instant Classical Mock)",
                    "Samyoga Shadow (Trainable Classical Twin)",
                    "Samyoga Legacy (Legacy Hybrid QTL + SVM)",
                ],
                style=get_q_style(),
            ).ask()
            if not sub_choice:
        ...
        if name == "parampara":
            sub_choice = questionary.select(
                "Which version of Parampara?",
                choices=["Parampara Pro (Tuned & Scalable)", "Parampara Legacy (Untuned)"],
                style=get_q_style(),
            ).ask()
            if not sub_choice:
                return
            if "Legacy" in sub_choice:
                ...
                sub_choice_2 = questionary.select(
                    "Which Parampara Pro Track?",
                    choices=[
                        "Parampara Pro (Fair Track - PCA Bounded)",
                        "Parampara Pro Industry (Industry Track - Unbounded)",
                    ],
                    style=get_q_style(),
                ).ask()
                if not sub_choice_2:
            ...
            sub_choice = questionary.select(
                "Which version of Ananta?",
                choices=["Ananta Pro (Enterprise-Grade Hybrid Extractor)", "Ananta Legacy (Uncalibrated VQC Strawman)"],
                style=get_q_style(),
            ).ask()
            if not sub_choice:
                return
    ...
    if name == "samyoga_pro":
        ...
        stats = {
            "Architecture": "TNQE + Mamba2 + QMoE",
            "Optimization": "QNG + DS-ZNE Error Mitigation",
            "NISQ Viability": "[bold red]Theoretical / Low[/bold red] (Requires Fault Tolerance)",
            "Complexity": "O(n^2 log d)",
        }
    elif name == "samyoga_go":
        ...
        stats = {
            "Architecture": "Pure Classical Mock",
            "Equivalent": "Samyoga Pro (Quantum)",
            "Training Time": "[bold green]Instant[/bold green]",
            "NISQ Viability": "N/A (Classical)",
        }
    elif name == "samyoga_shadow":
        ...
        stats = {
            "Architecture": "Classical MLP Mock",
            "Equivalent": "Samyoga Pro (Classical)",
            "Parameters": "Matched to Quantum",
            "NISQ Viability": "N/A (Classical)",
        }
        ...
        stats = {
            "Architecture": "Quantum Transfer Learning (Hybrid)",
            "Stage 1": "PennyLane VQC",
            "Stage 2": "Classical RBF SVM",
            "Qubits Used": "Depends on PCA",
            "NISQ Viability": "[bold green]Extremely High[/bold green]",
        }
        ...
        stats = {
            "Architecture": "Rigorous Classical Pipeline",
            "Tuning": "RandomizedSearchCV",
            "Features": "Unbounded (No PCA)",
            "NISQ Viability": "N/A",
        }
    elif name == "parampara":
        ...
        stats = {
            "Architecture": "Pure Quantum VQC",
            "Quantum Gates": "Rx, Ry, Rz, CNOT",
            "Layers": "3 to 5",
            "NISQ Viability": "[bold yellow]Moderate[/bold yellow]",
        }
    elif name == "ananta_pro":
        ...
        stats = {
            "Architecture": "Hybrid Extractor + RBF",
            "Features": "PCA + Quantum Z/Y/X",
            "Tuning": "RandomizedSearchCV",
            "NISQ Viability": "[bold green]Extremely High[/bold green]",
        }
    ...
    animate_panel(
        ...
        border_style=model_color(name),
        delay=0.01,
        header=Align.center(logo_table),
        footer=footer_group,
    )
    ...
    run_bench = questionary.confirm(
        f"Run a benchmark on {name.upper()} using '{STATE['dataset']}'?", style=get_q_style()
    ).ask()
    if run_bench:
        val_strategy = questionary.select(
            "Select Validation Strategy:",
            choices=[
                "Standard Split (80/20) - Fast baseline testing",
                "5-Fold CV - Robust statistical validation",
                "Monte Carlo (5 iterations) - Randomized resampling",
                "Leave-One-Out (LOOCV) - Maximum data utilization",
            ],
            style=get_q_style(),
        ).ask()
        ...
        with _make_progress() as progress:
            if "Standard" in val_strategy:
                ...
                if error_h[0]:
                    ...
                    return
                progress.update(
                    task, advance=max(0, 70 - progress.tasks[task].completed), description="Running predictions..."
                )
                ...
                progress.update(
                    task,
                    advance=100 - progress.tasks[task].completed,
                    description=f"[bold]✓ {name.upper()} Done![/bold]",
                )
                time.sleep(0.3)
                res_table = Table(
                    title=f"{name.upper()} Benchmark Results", show_header=True, header_style=get_color("success")
                )
                ...
                res_table = Table(
                    title=f"{name.upper()} {val_strategy} Results", show_header=True, header_style=get_color("success")
                )
                res_table.add_column("Dataset", justify="center")
...
def cmd_test(args):
    ...
    if not args:
        choices = [
            "Compare (Benchmark models against each other)",
            "Scaling (Test qubit scaling efficiency)",
            "Noise (Simulate quantum noise resistance)",
            "Ablation (Test Samyoga Legacy components)",
            "Stats (Generate Statistical Report)",
            "Quantum_advantage (Measure TNQE mathematical scaling)",
        ]
    ...
    if test_type in ["compare", "sweep"]:
        ...
        PROFILES = {
            "smoke": {"epochs": 1, "samples": 30, "qubits": 4, "binary": True},
            "demo": {"epochs": 3, "samples": 100, "qubits": 4, "binary": False},
            "fast": {"epochs": 3, "samples": 100, "qubits": 4, "binary": False},
            "low_data": {"epochs": 10, "samples": 200, "qubits": 8, "binary": False},
            "quantum_safe": {"epochs": 5, "samples": 200, "qubits": 4, "binary": True},
            "research": {"epochs": 10, "samples": 200, "qubits": 8, "binary": False},
            "large": {"epochs": ep, "samples": 1000, "qubits": 8, "binary": False},
            "full": {"epochs": ep, "samples": 999999, "qubits": 8, "binary": False},
        }
        ...
        if config["qubits"] > 8 or config["samples"] > 300 or config["epochs"] > 10:
            ...
            ans = questionary.confirm(
                "Suggested quantum-safe config: 4 qubits, N <= 200, epochs <= 5. Continue anyway?", style=get_q_style()
            ).ask()
            if not ans:
        ...
        fair_choice = questionary.select(
            ...
            choices=fair_choices,
            style=get_q_style(),
        ).ask()
        fair_mode = False
        if fair_choice and fair_choice.startswith("Let them"):
            console.print(
                "[warning]WARNING: Samyoga Pro uses Mamba-2 and QMoE. Pitting it against classical SVMs is structurally unfair. It will dominate accuracy but fail efficiency.[/warning]"
            )
        elif fair_choice:
            console.print(
                "[success]Fair Mode Activated: Using Classical Shadow equivalent for parameter-fair comparison.[/success]"
            )
            fair_mode = True
            if test_type == "sweep" or ds != "complexity_wall":
                console.print(
                    "[bold yellow]Recommendation: For a truly fair evaluation of quantum entanglement, test against the 'complexity_wall' dataset using /dataset.[/bold yellow]"
                )
        ...
        if test_type == "sweep":
            choices = [
                questionary.Choice(
                    f"{config.get('size_label', '')} {name}".strip(),
                    value=name,
                    checked=not config.get("is_large", False),
                )
                for name, config in DATASET_CONFIGS.items()
            ]
            datasets_to_run = questionary.checkbox(
                "Select datasets to include in the sweep (Space to toggle, Enter to confirm):",
                choices=choices,
                style=get_q_style(),
            ).ask()
        ...
        start_t = time.time()
        df = run_experiment_staged(
            run_comparison,
            {"datasets": datasets_to_run, "mode": mode, "fair_mode": fair_mode, "config": config},
            "Benchmarking models...",
        )
        end_t = time.time()
        if df is not None:
            ...
            df = df.sort_values(
                by=["Dataset", "Accuracy", "Efficiency", "Train_Time_s"], ascending=[True, False, False, True]
            )
            from rich.table import Table

            summary_table = Table(
                title=f"Experiment Summary: {target_name} ({mode}) [Seed-Wise]",
                show_header=True,
            ...
            avg_table = Table(
                title="Combined Mathematical Averages (All Seeds)",
                show_header=True,
                header_style=get_color("secondary"),
            )
            ...
            for i, (_, row) in enumerate(avg_df.iterrows()):
                ...
                avg_table.add_row(
                    rank_str,
                    f"[bold {model_color(m_name)}]{m_name}[/]",
                    f"{row['Accuracy']:.2%}",
                    f"{row['Train_Time_s']:.2f}s",
                    f"{row['Efficiency']:.4f}",
                )
            console.print()
            ...
            pts_table = Table(
                title="Aggregate Championship Standings", show_header=True, header_style=get_color("accent")
            )
            ...
            console.print(
                f"[bold {get_color('success')}]✓ Leaderboard automatically exported to {csv_path} for your research paper.[/bold {get_color('success')}]"
            )
            console.print()
            try:
                from ui.graphs import render_rich_metric_bars, save_accuracy_vs_time_scatter, save_all_comparison_charts
        ...
        df = run_experiment_staged(
            run_scaling,
            {"datasets": [ds], "config": {"epochs": ep, "qubits": 4, "binary": False}},
            "Training across sizes...",
        )
        if df is not None:
        ...
        df = run_experiment_staged(
            run_noise,
            {"datasets": [ds], "config": {"epochs": ep, "samples": 100, "qubits": 4, "binary": False}},
            "Simulating noise...",
        )
        if df is not None:
        ...
        df = run_experiment_staged(
            run_ablation,
            {"datasets": [ds], "config": {"epochs": ep, "samples": 100, "qubits": 4, "binary": False}},
            "Testing Samyoga Legacy components...",
        )
        if df is not None:
        ...
        print_panel("Generating Statistics Report...", style_key="success")
        try:
        ...
        print_error_panel(
            "Unknown test",
            f"'{test_type}' is invalid.",
            "Available: compare, scaling, noise, ablation, stats.",
            "/test compare",
        )
...
def print_categorical_comparisons(df):
    ...
    categories = {
        ...
        "Pro/Pro+ Heavyweights": df[
            df["Model"].str.lower().str.contains(r"pro(\+)?\b", regex=True)
            & ~df["Model"].str.lower().str.contains("legacy")
        ].copy(),
    ...
    for title, cat_df in categories.items():
        ...
        header_txt = gradient_text(
            "--- " + title + " ---",
            get_color("secondary").replace("bold ", ""),
            get_color("accent").replace("bold ", ""),
        )
        from rich.align import Align
...
def cmd_results():
    ...
    from ui.components import df_to_table, print_interpretation, print_winner_podium
...
def cmd_graphs(args=None):
    ...
    if args:
        ...
        if action == "list":
            ...
            print_error_panel(
                "Unknown argument", f"'{action}' not valid.", "Available: list, latest, summary", "/graphs list"
            )
            return
    if not choice:
    ...
    if not choice or choice == "Cancel":
        return
    from ui.graphs import render_rich_metric_bars, render_terminal_curve
    ...
    if choice == "Show latest comparison bars":
        ...
        render_rich_metric_bars(
            "Latest Accuracy Comparison",
            models,
            df_latest["accuracy"].tolist(),
            "Accuracy",
            datasets=ds_list_latest,
            seeds=seed_list_latest,
        )
        ...
        if os.path.exists(figs_dir):
            ...
            if files:
                console.print(
                    f"[bold {get_color('primary')}]Saved Figures in {figs_dir}:[/bold {get_color('primary')}]"
                )
                for f in files:
                    console.print(f"  - {f}")
            else:
        ...
        render_rich_metric_bars(
            "Average Train Time Summary", models, grouped["time_s"].tolist(), "Time (s)", higher_is_better=False
        )
        curve_dict = {}
        for m in df["model"].unique():
            accs = df[df["model"] == m]["accuracy"].tolist()
...
def cmd_theme(args):
    ...
    restart = questionary.confirm(
        "Would you like to restart Darshan to fully apply this theme?", style=get_q_style()
    ).ask()
    if restart:
        console.clear()
        boot_sequence()
...
def cmd_working(args):
    ...
    if action == "on":
        ...
        console.print(
            Panel(
                STATE["working_log"][-2000:] if STATE["working_log"] else "No logs captured yet.",
                title="Working Logs (Last 2000 chars)",
                border_style="dim",
            )
        )
    elif action == "clear":
...
def cmd_reset():
    ...
    from ui.components import print_error_panel, print_panel
    from ui.theme import get_q_style
...
def cmd_about():
    ...
    grid.add_row(
        "Low-Data Regime Advantage",
        "We hypothesize that when training data is severely limited (N < 200), Quantum Transfer Learning can capture higher-dimensional correlations and outperform both purely classical SVMs and purely quantum classifiers.",
    )
    grid.add_row(
        "Kernel-VQC Duality",
        "The suite evaluates the intrinsic duality between Variational Quantum Classifiers (VQCs) and Quantum Kernel methods, demonstrating that architectural distinctions are secondary to optimization landscapes.",
    )
    grid.add_row(
        "Shadow QML Deployment",
        "Darshan explores deployment strategies where quantum circuits learn complex embeddings during training, but actual inference is performed classically to reduce commercial scalability barriers.",
    )
    grid.add_row(
        "Hardware-Compatible Encoding",
        "The suite aggressively benchmarks angle-encoding and PCA-reduced feature strategies that fit current physical limits, while systematically validating model robustness against simulated depolarizing noise.",
    )
    console.print()
...
def repl():
    ...
    datasets = {
        k: None for k in sorted(["Iris", "Wine", "Moons", "Breast Cancer", "Digits", "Pendigits", "Complexity Wall"])
    }
    models = {
        k: None
        for k in sorted(
    ...
    while True:
        try:
            ...
            get_color("secondary").replace("bold ", "").replace("dim ", "")
            ...
            style = Style.from_dict(
                {
                    "completion-menu.completion": "bg:#141414",
